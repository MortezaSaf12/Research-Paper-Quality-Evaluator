...
`;
    ...
    return {
      success: false,
      error: error.message || "Error comparing papers with Gemini"
    };
  }
}
  ...
  async comparePapersPairwise(files) {
    try {
      ...
      console.error("Gemini pairwise comparison error:", error);
      return {
        success: false,
        error: error.message || "Error comparing papers with Gemini"
      };
  ...
  `;
      ...
      let winner;
      if (response.toLowerCase().includes("paper 1") && !response.toLowerCase().includes("paper 2 has higher")) {
        winner = 0;
      } else if (response.toLowerCase().includes("paper 2") && !response.toLowerCase().includes("paper 1 has higher")) {
        winner = 1;
      } else {
        winner = 0.5; // Tie
      }
