...
const gpt4Service = new GPT4Service(); // Using GPT4Service instead of GeminiService
...
exports.evaluatePaper = async (req, res) => {
  try {
    ...
    const evaluationResult = await gpt4Service.evaluatePaper(file);
...
exports.evaluateAndComparePapers = async (req, res) => {
  try {
    ...
    if (files.length === 1) {
      comparisonResult = await gpt4Service.evaluatePaper(files[0]);
    } else {
      comparisonResult = await gpt4Service.comparePapers(files);
    }
